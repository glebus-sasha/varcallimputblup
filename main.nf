...
workflow test { 
    ...
    GLIMPSE2_SPLITREFERENCE(ref_panel.join(ref_panel_index).combine(IRG_ORG))
    GLIMPSE2_PHASE(
        GLIMPSE2_SPLITREFERENCE.out.bin_ref.map{it->it[1]},
        ref_panel_index, 
        bam.join(bamindex)
        )
    GLIMPSE2_LIGATE(GLIMPSE2_PHASE.out.phased_variants, GLIMPSE2_PHASE.out.list)
}
